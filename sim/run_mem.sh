#!/bin/sh
# Pocket memory gate: target/pocket/theglob_mem.sv, the real SDRAM controller and
...
verilator --cc --exe --build -j "${JOBS:-8}" -O2 \
    ...
    ../target/pocket/theglob_mem.sv ../target/pocket/sdram_ctrl.sv ../target/pocket/sram_port.sv \
