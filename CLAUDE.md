...
`rtl/theglob_core.sv` with the machine, without ever breaking what already
works around it.
...
2. **Read MAME's driver and devices** into `ref/mame/` (verbatim, fetched) and
   ...
   yet.
3. **The ROM image.** Write `theglob.mra`, build the image with
   ...
   `target/pocket/theglob_mem.sv` and `sim/tb_mem.cpp` to match, and run
   `sim/run_mem.sh`.
...
7. **The whole machine on the real memory glue** — `sim/run_system.sh`, which
   ...
   `theglob_mem`, real SDRAM controller and SRAM port, behavioural chips beyond
...
|---|---|---|
| The Pocket's loader cannot wait; a single pending download word corrupts the image | 64-word FIFO, edge-detected strobe, in `theglob_mem.sv`; `sim/run_mem.sh` fails without it | 5.16 |
...
rtl/theglob_core.sv                      THE MACHINE.  skeleton now; ports are the contract
...
target/pocket/theglob_mem.sv             SDRAM clients, download FIFO, burst arbiter, SRAM
...
sim/run_mem.sh -quick        # a minute; after touching theglob_mem.sv (drop -quick before a flash)
